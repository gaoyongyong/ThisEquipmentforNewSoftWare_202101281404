...
namespace MyCpk
{
    ...
    public static class CreateTableService
    {
            ...
            foreach (PropertyInfo propertyInfo in propertyInfos)
            {
                switch (propertyInfo.PropertyType.Name)
                {
                    case "String":
                        ...
                        {
                            createSql += "`" + propertyInfo.Name + "` VARCHAR(200) NOT NULL,";
                        }
                        else
                        {
                            createSql += "`" + propertyInfo.Name + "` VARCHAR(200) NULL,";
                        }
                        break;
                ...
                else if (columnProperties1.Count == 0)
                {
                    switch (propertyInfo.PropertyType.Name)
                    {
                        case "String":
                            if (propertyInfo.Name == "ID")
                            {
                                updateSql += "ADD COLUMN `" + propertyInfo.Name + "` VARCHAR(200) NOT NULL,";
                            }
                            else
                            {
                                updateSql += "ADD COLUMN `" + propertyInfo.Name + "` VARCHAR(200) NULL,";
                            }
                            break;
        ...
        public static void CheckSchemaExist(string DataBaseName)
        {
            ...
            {
                string creatSql = "CREATE SCHEMA `aphz_autolinedatabase`";
                bool result = MySqlTool.CreateSchema(creatSql);
                if (!result)
                    MessageBox.Show("Create Schema Fail!");
            }
        ...
        public static void CheckTableExist(List<Type> Entities,string DataBaseName)
        {
            ...
            {
                string checkSql = "SELECT * FROM information_schema.Tables WHERE TABLE_NAME = '" + entity.Name + "'";
                try
                {
